...
cout<<"enter roll last 3 digits= ";
cin>>roll;


cout<<"\nofstream.xls created copy column A paste to autocad 16 or higher command box\n";
if(roll<=50)
    q=roll+20000;
else if(roll<=100)
    q=roll+18000;
    ...
    cout<<"\ninput discharge in cumec y/n = ";
    char deci;
    cin>>deci;
    if(deci=='y')
    {
        cout<<"\nenter discharge in cumec = ";
        cin>>q;
    }

cout<<"\nwidth of the stilling basin (such as 200m) B = ";
cin>>b;

cout<<"\nvelocity at upstream (such as 22m/sec) = ";
cin>>u1;

cout<<"\ntail water depth (such as 18m) = ";
cin>>Tw;
cout<<"\nEnter name on drawing  = ";
cin>>name;
cout<<"\nEnter std id full on drawing  = ";
cin>>id;
...
xl.close();
dl.close();
system("drawing_codes_for_autocad_16_or_higher.txt");
//system("base.exe");
